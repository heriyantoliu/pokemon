...
define root view entity ZPK_R_POKE_EVO_REL
  as select from zpk_poke_species

  association of many to one ZPK_R_POKE_EVO_REL as _Origin  on $projection.EvolvesFromSpeciesId = _Origin.ID
  association [0..1]  to     ZPK_I_Pokemon      as _Pokemon on $projection.ID = _Pokemon.ID
